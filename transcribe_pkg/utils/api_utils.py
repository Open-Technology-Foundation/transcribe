...
def _is_reasoning_model(model: str) -> bool:
    """
    Check if a model is a reasoning model (GPT-5, o1, o3 series).
...
class OpenAIClient:
    ...
    def transcribe_audio(
        ...
        try:
            ...
            raise APIRateLimitError(f"OpenAI API rate limit exceeded: {str(e)}") from e

        except openai.AuthenticationError as e:
            self.logger.error(f"Authentication error: {str(e)}")
            raise APIAuthenticationError(f"OpenAI API authentication error: {str(e)}") from e

        except openai.APIConnectionError as e:
            self.logger.warning(f"API connection error: {str(e)}")
            raise APIConnectionError(f"OpenAI API connection error: {str(e)}") from e
    ...
    def chat_completion(
        ...
        Note:
            GPT-5 and reasoning models (o1, o3) do not support custom temperature
            (only default of 1) and use max_completion_tokens instead of max_tokens.
        """
        messages = [
        ...
        if _is_reasoning_model(model):
            ...
            # Don't set temperature for reasoning models - they only support default (1)
        ...
        try:
            ...
            if not response.choices or not response.choices[0].message.content.strip():
                ...
                if _is_reasoning_model(model):
                    self.logger.debug(f"Empty response from reasoning model: user_prompt='{user_prompt[:128]}...'")
                else:
...
openai_client: OpenAIClient | None = None  # For test compatibility
...
def call_llm(user_prompt: str, system_prompt: str | None = None, model: str = "gpt-4o",
    ...
    Note:
        ...
        GPT-5 and reasoning models (o1, o3 series):
    ...
    if openai_client is not None and hasattr(openai_client, 'chat'):
        ...
        # Handle GPT-5 models - use max_completion_tokens instead of max_tokens
        # and do not support custom temperature (only default of 1)
        if _is_reasoning_model(model):
            # Don't set temperature for reasoning models - they only support default (1)
        ...
        try:
            response_obj = openai_client.chat.completions.create(**api_params)
            ...
            if "choices" in str(e).lower() or "empty" in str(e).lower():
                raise EmptyResponseError(f"Empty response from API: {str(e)}") from e
            else:
...
def transcribe_audio(audio_file: str | BinaryIO, model: str = "whisper-1",
...
def _transcribe_audio_impl(audio_file: str | BinaryIO, model: str = "whisper-1",
                          prompt: str = "", language: str = 'en',
                          response_format: str = "text", temperature: float = 0.05) -> Any:
    """
