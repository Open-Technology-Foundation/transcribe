...
def generate_srt(segments, max_line_length=42, max_duration=5.0):
    """
...
def generate_vtt(segments, max_line_length=42, max_duration=5.0):
    """
...
def save_subtitles(transcript_result, output_path, format_type="srt"):
    """
    ...
    if not segments:
        logging.error("No segments with timestamps found in transcript")
        return None
    ...
    try:
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(content)
        logging.info(f"Subtitle file saved to: {output_path}")
        return output_path
    except Exception as e:
        logging.error(f"Error saving subtitle file: {str(e)}")
        return None
